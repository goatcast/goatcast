...
export function Sidebar({ selectedDesk, onDeskSelect, profile, isExpanded = true, onToggleExpand }) {
	...
	return (
		<>
			<div className={`${isExpanded ? 'w-64' : 'w-16'} bg-white dark:bg-neutral-950 border-r border-gray-200 dark:border-neutral-800 h-screen flex flex-col overflow-y-auto transition-all duration-300`}>
				{/* Header */}
				<div className={`${isExpanded ? 'p-4' : 'p-2'} border-b border-gray-200 dark:border-neutral-800`}>
					<div className={`flex items-center ${isExpanded ? 'justify-between' : 'flex-col gap-2'} mb-4`}>
						...
						<button
							onClick={toggleTheme}
							className={`${isExpanded ? 'p-2' : 'w-10 h-10 p-0'} rounded-md hover:bg-gray-100 dark:hover:bg-neutral-800 transition-colors flex items-center justify-center`}
							title={
								theme === 'light'
									? 'Switch to dark mode'
					...
					{/* Toggle Button */}
					<div className={`flex items-center ${isExpanded ? 'gap-2' : 'justify-center'} mb-4`}>
						<button
							onClick={onToggleExpand}
							className={`${isExpanded ? 'w-full px-4 py-2' : 'w-10 h-10 p-0'} bg-gray-100 dark:bg-neutral-800 hover:bg-gray-200 dark:hover:bg-neutral-700 text-gray-700 dark:text-gray-300 font-semibold rounded-md transition-colors duration-200 text-sm flex items-center justify-center ${isExpanded ? 'gap-2' : ''}`}
				...
				<div className={`flex-1 overflow-y-auto ${isExpanded ? 'p-4' : 'p-2'}`}>
					<div className={`${isExpanded ? 'space-y-2' : 'space-y-2 flex flex-col items-center'}`}>
						{loading ? (
							<div className={`text-gray-500 dark:text-neutral-400 text-sm text-center py-4 ${!isExpanded ? 'hidden' : ''}`}>
								Loading desks...
							</div>
						) : desks.length === 0 ? (
							<div className={`text-gray-500 dark:text-neutral-400 text-sm text-center py-4 ${!isExpanded ? 'hidden' : ''}`}>
							...
							desks.map((desk) => (
								...
								>
									<button
										...
										className={`w-full ${isExpanded ? 'text-left px-4 py-3' : 'text-center p-0 w-8 h-8'} text-gray-900 dark:text-white font-medium flex items-center ${isExpanded ? 'justify-between' : 'justify-center'} group`}
									...
									>
										{isExpanded ? (
											...
											<span className="text-lg font-bold">{desk.name.charAt(0).toUpperCase()}</span>
										)}
									</button>
								</div>
							))
				...
				{/* User Profile */}
				{profile && (
					<div className={`${isExpanded ? 'p-4' : 'p-2'} border-t border-gray-200 dark:border-neutral-800`}>
						<div className={`flex items-center ${isExpanded ? 'gap-3' : 'justify-center'}`}>
							{profile.pfpUrl && (
								<img
									src={profile.pfpUrl}
									alt={profile.displayName || profile.username || 'User avatar'}
									className={`${isExpanded ? 'w-10 h-10' : 'w-10 h-10'} rounded-full object-cover`}
